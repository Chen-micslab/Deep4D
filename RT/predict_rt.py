...
def predict_nolabel_rt(args):
    slice = args.slice
    for slice_num in range(slice):
        print(f'{slice_num}/{slice} slices')
        data_dir = f'./dataset/data/{args.filename}{slice_num}'
        torch.cuda.manual_seed(args.seed)
        torch.manual_seed(args.seed)
        np.random.seed(args.seed)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  ##判断使用GPU还是CPU
        ...
        data.to_csv(f'./dataset/data/output/{args.filename}{slice_num}_rt_pre.csv', index=False)  # 保存所有的预测值和真实值到一个CSV文件
