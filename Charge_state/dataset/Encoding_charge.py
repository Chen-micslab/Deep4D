...
def encoding_without_label(input, output):
    ...
    sinusoid_table = np.array(
        [get_position_angle_vec(pos_i, 23) for pos_i in range(50)])
    ...
    for i in range(len(peptide_list)):
        ...
        if peptide_list[i][0] == 'a':
            ...
            pep_len = len(peptide_list[i])
        onehot_arrary = peptide_onehot_encoding(peptide_list[i], sinusoid_table)
        num_id = str(num).zfill(7)
        if pep_len < 50:
            np.save(f'{output}_onehot/{num_id}_{peptide_list[i]}_${pep_len}$.npy', onehot_arrary)
...
def encoding_with_label(input, output):
    ...
    num = 0
    peptide_list = data['Peptide']
    if 'Charge' in list(data):
        charge_list = data['Charge']
    else:
        charge_list = [2] * len(peptide_list)
    sinusoid_table = np.array(
        [get_position_angle_vec(pos_i, 23) for pos_i in range(50)])  # 先计算position encoding sin和cos里面的值
