...
class Mydata_label(Dataset):
    def __init__(self, datadir):
        self.msms_dir = f'{datadir}_msms' ##这里data_dir是文件的储存路径
    ...
    def __getitem__(self, index):
        ...
        peptide_msms = np.load(peptide_msms_dir)  
        peptide_len = int(self.extract(peptide_index,'$')) 
        peptide_charge = int(self.extract(peptide_index,'#'))
        peptide = self.extract(peptide_index,'_')
        sample = {'peptide_onehot':peptide_onehot, 'peptide_msms':peptide_msms,'length':peptide_len,'charge':peptide_charge, 'peptide':peptide}
        return sample
...
class Mydata_nolabel(Dataset):
    ...
    def __getitem__(self, index):
        ...
        peptide_charge = int(self.extract(peptide_index,'#'))
        peptide = self.extract(peptide_index,'_')
        sample = {'peptide_onehot':peptide_onehot, 'length':peptide_len,'charge':peptide_charge, 'peptide':peptide}
        return sample
