...
class Transformer(Module):
    ...
    def __init__(self, feature_len=23, max_len=50, d_model: int = 512, charge_lenth: int = 100 , nhead: int = 8, num_encoder_layers: int = 6,
        ...
        self.linear1 = Linear(feature_len, (d_model-charge_lenth))  
        self.linear2_1 = Linear(1, max_len)
        self.linear2_2 = Linear(1, charge_lenth)
        ...
        self.linear3 = Linear(5 * 24 * 124, 49*12)  


    def forward(self, src, src_mask=None, src_key_padding_mask=None, charge=None) -> Tensor:
        src = src.transpose(0,1)
        src1 = self.linear1(src)
        charge = charge.unsqueeze(1).unsqueeze(1)
        src2 = self.linear2_1(charge).transpose(1,2)
        src2 = self.linear2_2(src2).transpose(0,1)
        src = torch.cat([src1,src2],2)
