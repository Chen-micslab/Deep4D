...
class Mydata(Dataset):
    ...
    def __getitem__(self, index):
        ...
        CCS = self.extract(peptide_index,'_')/10000  
        peptide_len = self.extract(peptide_index,'$') 
        peptide_charge = self.extract(peptide_index,'#')
        sample = {'peptide':peptide,'CCS':CCS,'length':peptide_len,'charge':peptide_charge}
        return sample
