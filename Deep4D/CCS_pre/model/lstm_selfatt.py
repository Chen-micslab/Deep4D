...
class Transformer(Module):
    ...
    def __init__(self, feature_len=23, max_len=50, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6,
        ...
        self.lstm = nn.LSTM(input_size=self.d_model, hidden_size=int(d_model/2), num_layers=2, bidirectional=True,
        ...
        self.linear2 = Linear(max_len*d_model, 1000)  
        self.linear3 = Linear(1000, 100)  
        self.linear4 = Linear(100, 23)
        self.linear5 = Linear(23, 1)

    def forward(self, src, src_mask=None, src_key_padding_mask=None, charge=None,pos=None) -> Tensor:
        src = self.linear1(src)
        batch = src.size()[1]  
        h_0 = torch.randn(2*2, batch, int(self.d_model/2)).to(device='cuda', dtype=torch.float32)
        c_0 = torch.randn(2*2, batch, int(self.d_model/2)).to(device='cuda', dtype=torch.float32)
        pep , (h_n,c_n) = self.lstm(src,(h_0,c_0))
        pep = pep + pos
        if src.size(2) != self.d_model:
            raise RuntimeError("the feature number of src must be equal to d_model")
        pep = self.encoder1(pep, mask=src_mask, src_key_padding_mask=src_key_padding_mask)
