...
from model.selfatt_cnn_ccs import Transformer
...
def predict_nolabel_ccs(args,task_dir = None):
    ...
    for slice_num in range(slice):
        ...
        data.to_csv(f'{file_dir}/output/{args.filename}_slice{slice_num}_pre_ccs.csv', index=False)  # 保存所有的预测值和真实值到一个CSV文件
