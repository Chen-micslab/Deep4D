...
def get_args():  ##设置需要传入的参数
    ...
    parser.add_argument('--Merged_predict', type=bool, default=False)
...
def predict_nolabel_msms(args, task_dir = None):
    ...
    for slice_num in range(slice):
        ...
        data = np.column_stack((pep, charge_list, m_z_list, pep_len, total_peptide_msms_pre_1))
        data = pd.DataFrame(data)
        data.to_csv(f'{file_dir}/output/{args.filename}_slice{slice_num}_pre_msms.csv', index=False)
        # data1 = np.column_stack((pep, charge_list))
        # data1 = pd.DataFrame(data1, columns=['Peptide', 'Charge'])
        # data1.to_csv(f'{file_dir}/output/{args.filename}_RTCCS_input_slice{slice_num}.csv', index=False)

if __name__ == '__main__':
    args = get_args()  ##生成参数列表
    if args.Merged_predict:
        print('Erorr: This program does not support merged predict!!! please use Merged_predict.py')
    else:
        if args.label == 1:
