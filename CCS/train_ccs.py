...
def get_args():  ##设置需要传入的参数
    parser = argparse.ArgumentParser(description='Train the ccs model')
...
def train(model,device,epochs=10, batch_size=1,lr=0.001,val_percent=0.1,save_mp=True, sch = 0,
    ...
    if sch==0:
         scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=optimizer,T_0=30,T_mult=2,eta_min=0.00000001)
    else:
