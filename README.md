...
The training data should be stored in a comma-separated values (CSV) file including two column:'Peptide', 'Charge', 'CCS'. This CSV file should be stored at the directory 'Deep4D/Deep4D/dataset/data/ccs_data.csv'.
```
Peptide,Charge,CCS
...
```
python train_ccs.py --filename 'ccs_data' --load_param_dir './checkpoint/ccs.pth' --lr 0.00001 --batch_size 50
```
...
### b. Train rt model  
#### 1. Prepare the training data
The training data should be stored in a comma-separated values (CSV) file including two column:'Peptide', 'RT'. This CSV file should be stored at the directory 'Deep4D/Deep4D/dataset/data/rt_data.csv'.
```
Peptide,RT
